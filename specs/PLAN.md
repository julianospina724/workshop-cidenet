...
**Verificado:** 49/49 tests en verde (6 nuevos en `PermissionsPage.test.tsx`, todos en verde a la primera). Matriz con roles fijos (Admin/Editor/Viewer) × recursos fijos (users/roles/permissions/reports) × acciones CRUD — sin ninguna opción de UI para agregar rol o recurso nuevo. Los toggles solo modifican estado local (borrador); "Guardar cambios" calcula el diff contra el último estado persistido y envía solo los cambios reales al backend. La fila del propio rol del Admin aparece deshabilitada (con tooltip explicando R3), reforzando en UI la regla que el backend ya rechaza server-side. Fallo general al guardar revierte los toggles a su último estado guardado y muestra el mensaje genérico. Enlace "Matriz de permisos" agregado al header (solo Admin). Verificado visualmente con Playwright contra Docker: login como Admin sembrado, desactivar el permiso de crear reports para Editor, guardar, confirmación visible, persistencia tras recargar, y restauración del valor original para dejar los datos sembrados intactos.
